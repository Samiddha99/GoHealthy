...
$("#degree").on('change', loadSpeciality);
function loadSpeciality()
{
    ...
    $.ajax(
    {
        ...
        success: function(response)
        { 
            $("#speciality").html(`<option label="Select Speciality" selected></option>`);

            for(var i in response.speciality)
            {
                $("#speciality").append(`<option value="${response.speciality[i].id}">${response.speciality[i].Speciality}</option>`);
            }
        },
    });
}
...
$('#contacteditform').on('submit', function(e) {
    ...
    {
        document.getElementById('submit_btn').disabled = true;
        document.getElementById('submit_btn').innerHTML = "<i class='fad fa-spinner fa-spin'></i> Submitting";
        $.ajax(
        {
            type:'POST',
            url: $(this).attr('action'),
